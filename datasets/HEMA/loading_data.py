...
def HEMA_collate_multibatch(batch):
    ...
    if isinstance(imgs[0], torch.Tensor) and isinstance(dens[0], torch.Tensor):
        ...
        for i_sample in range(len(batch)):
            ...
            _den = F.conv2d(_den, filter, stride=8)
            _den = _den.squeeze()
...
def HEMA_collate_onebatch(batch):
    transposed = list(zip(*batch))  # imgs and dens
    imgs, dens = [transposed[0], transposed[1]]
...
def loading_data():
    ...
    train_set = HEMA(cfg_data.TRAIN_DATA_PATH,
                     main_transform=train_main_transform,
                     img_transform=img_transform,
                     gt_transform=gt_transform,
                     k_size=cfg_data.K_SIZE,
                     sigma=cfg_data.SIGMA,
                     exclude_invalid=cfg_data.EXCLUDE_INVALID)

    train_loader = DataLoader(train_set,
                              batch_size=cfg_data.TRAIN_BATCH_SIZE,
                              num_workers=0,
                              collate_fn=HEMA_collate_multibatch,
                              shuffle=True,
                              drop_last=True)

    val_set = HEMA(cfg_data.VAL_DATA_PATH,
                   main_transform=None,
                   img_transform=img_transform,
                   gt_transform=gt_transform,
                   k_size=cfg_data.K_SIZE,
    ...
    val_loader = DataLoader(val_set,
                            ...
                            num_workers=0,
                            collate_fn=HEMA_collate_onebatch,
                            shuffle=True,
                            drop_last=False)
